...
/**
 * Customized RestRouter
 *
 * @author ayakurayuki
 * @date 2019/06/06-10:36
 */
public class RestRouter {
  ...
  private final static Logger log = LoggerFactory.getLogger(com.zandero.rest.RestRouter.class);
  ...
  public static ConcurrentHashMap<String, Boolean> authUrlMap = new ConcurrentHashMap<>();
  ...
  public static Router register(Router router, Object... restApi) {
    ...
    for (Object api : restApi) {
      ...
      for (RouteDefinition definition : definitions.keySet()) {
        ...
        Annotation[] annotations = definitions.get(definition).getAnnotations();
        ...
        if (definition.checkSecurity() || checkToken) {
          route.handler(getSecurityHandler(definition));
          authUrlMap.put(route.getPath(), true);
        }
        ...
        log.info("Registered route: " + definition + (checkToken ? " * Require token" : ""));
      }
    }
  ...
  public static void handler(Router output, Class<? extends Handler<RoutingContext>> handler) {
    ...
    try {
      Handler<RoutingContext> instance = (Handler<RoutingContext>) ClassFactory
          .newInstanceOf(handler, injectionProvider, null);
  ...
  private static HttpResponseWriter getWriter(InjectionProvider injectionProvider,
    ...
    HttpResponseWriter writer = writers
        .getResponseWriter(returnType, definition, injectionProvider, context, acceptHeader);
  ...
  private static Handler<RoutingContext> getSecurityHandler(final RouteDefinition definition) {
    ...
    return context -> {
      ...
      boolean allowed = isAllowed(user, definition);
      if (allowed) {
        context.next();
      } else {
  ...
  private static Handler<RoutingContext> getHandler(final Object toInvoke, final RouteDefinition definition,
    ...
    return context -> context.vertx().executeBlocking(
        fut -> {
          try {
            Object[] args = ArgumentProvider
                .getArguments(method, definition, context, readers, providers, injectionProvider);
  ...
  private static Handler<RoutingContext> getAsyncHandler(final Object toInvoke, final RouteDefinition definition,
      final Method method) {
            ...
            if (fut.succeeded()) {
              ...
              try {
                ...
                if (futureResult != null) { // get writer from result type otherwise we don't know
                  ...
                  Class<?> writerClass =
                      definition.getWriter() == null ? GenericResponseWriter.class : definition.getWriter();
  ...
  private static void validate(Method method, RouteDefinition definition, Validator validator, Object toInvoke,
      Object[] args) {
  ...
  private static void validateResult(Object result, Method method, RouteDefinition definition, Validator validator,
      Object toInvoke) {

    if (validator != null) {
      ExecutableValidator executableValidator = validator.forExecutables();
      Set<ConstraintViolation<Object>> validationResult = executableValidator
          .validateReturnValue(toInvoke, method, result);
    ...
    return context -> {
      ...
      try {
        ...
        if (notFoundWriter instanceof Class) {
          writer = writers
              .getClassInstance((Class<? extends HttpResponseWriter>) notFoundWriter, injectionProvider, context);
        } else {
